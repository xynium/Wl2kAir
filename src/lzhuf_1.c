...
static int GetBit(struct qzbuffer *inbuf);
static int GetByte(struct qzbuffer *inbuf);
static int Putcode(int l, unsigned c, struct qzbuffer *outbuf);
static void StartHuff(void);
static void reconst(void);
static void update(int c);
static int EncodeChar(unsigned c, struct qzbuffer *outbuf);
static int EncodePosition(unsigned c, struct qzbuffer *outbuf);
static int EncodeEnd(struct qzbuffer *outbuf);
static int DecodeChar(struct qzbuffer *inbuf);
static int DecodePosition(struct qzbuffer *inbuf);
static void linko (int n, int p, int q);
...
static int
crc_fputc(int c, struct qzbuffer *crcoutbuf)
{
...
static int
crc_fgetc(struct qzbuffer *crcinbuf)
{
  int retour;
...
static int
Putcode(int l, unsigned c, struct qzbuffer *outbuf)
{
  putbuf |= c >> putlen;
  if ((int)(putlen += l) >= 8) {
...
static int
EncodeChar(unsigned c, struct qzbuffer *outbuf)
{
  unsigned i;
  int j, k;
...
static int
EncodePosition(unsigned c, struct qzbuffer *outbuf)
{
  unsigned i;
...
static int
EncodeEnd(struct qzbuffer *outbuf)
{
  if (putlen) {
    if (crc_fputc((int) (putbuf >> 8), outbuf) == EOF) {
      return -1;
...
static int
DecodeChar(struct qzbuffer *inbuf)
{
  unsigned c;
...
int
DecodePosition(struct qzbuffer *inbuf)
{
  unsigned i, j, c;
...
/* compression */
struct qzbuffer *
Encode(struct qzbuffer *inbuf)
{
  int i, c, len, r, s, last_match_length;
#ifdef LZHUF_1_MAIN
  unsigned int printcount;
#endif
  struct qzbuffer *outbuf;
